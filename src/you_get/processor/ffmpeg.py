...
from ..common import ffmpeg_location
...
def get_usable_ffmpeg(cmd):
    try:
        ...
        try:
            ...
            version = [1, 0]
        return cmd, os.path.join(ffmpeg_location, 'ffprobe'), version
    except:
        return None

FFMPEG, FFPROBE, FFMPEG_VERSION = get_usable_ffmpeg(os.path.join(ffmpeg_location, 'ffmpeg')) or get_usable_ffmpeg('avconv') or (None, None, None)
...
def ffmpeg_concat_mp4_to_mpg(files, output='output.mpg'):
    ...
    params += ['-vcodec', 'copy', '-acodec', 'copy','-strict','-2'] # strict -2 ,允许使用实验性特性（opus允许嵌入mp4容器）
    params.append(output)
