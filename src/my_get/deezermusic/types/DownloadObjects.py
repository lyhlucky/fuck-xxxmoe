...
class IDownloadObject:
    ...
    def updateProgress(self, listener=None):
        self.progress = self.progressNext
        if listener: listener.send("updateQueue", {
            'progress': self.progress,
            'speed': self.progressSpeed,
            'filesize': self.fileSize,
            'eta': self.progressRemainingTime
        })
...
class Single(IDownloadObject):
    ...
    def completeTrackProgress(self, listener=None): ## 完整进度
        self.progressNext = 100
        self.updateProgress(listener)

    def removeTrackProgress(self, listener=None): ## 清空进度
        self.progressNext = 0
        self.updateProgress(listener)  ## 更新进度
