...
from .types.Picture import StaticPicture
from .utils import USER_AGENT_HEADER
from .utils.pathtemplates import generatePath, generateAlbumName, generateArtistName, generateDownloadObjectName
from .tagger import tagID3, tagFLAC
from .decryption import generateCryptedStreamURL, streamTrack
from .settings import OverwriteOption
from .errors import DownloadFailed, MD5NotFound, DownloadCanceled, PreferredBitrateNotFound, TrackNot360, AlbumDoesntExists, DownloadError, ErrorMessages
import os
...
class Downloader:
    def __init__(self, dz, downloadObject, settings, listener=None ,params = None):
        self.dz = dz
    ...
    def download(self, extraData, track=None):
        ...
        playlist = None
        if albumAPI is not None:
            playlist = albumAPI
        elif playlistAPI is not None:
            playlist = playlistAPI

        if playlist is not None:
            medias = []
            for playlist_track in playlist['tracks']['data']:
                medias.append({
                    'title': playlist_track['title'],
                    'url': playlist_track['link']
                })
        ...
        if self.params['add_playlist_index'] == "true":
            writepath = filepath / f"{self.params['playlist_index']}.{filename}{extension}"
        else:
        ...
        self.log(data={
            'ret_code': '0',
            'title': track.title,
            'thumbnail': track.album.embeddedCoverURL,
            'local_thumbnail': '',
            'duration': track.duration,
            'is_live': False
        }, state="downloadStart")

        self.writepath = str(writepath)
