...
class SimpleProgressBar:
    ...
    def __init__(self, total_size, total_pieces=1, progress_hook=None):
    ...
    def update_received(self, n):
        ...
        if bytes_ps > 0:
            self.eta = int((self.total_size - self.received) / bytes_ps)
        self.last_updated = time.time()
        self.update()
...
def download_urls(
    ...
    if total_size:
        if not force and os.path.exists(output_filepath) and not auto_rename\
            ...
            print()
            return
        bar = SimpleProgressBar(total_size, len(urls), progress_hook=kwargs['progress_hook'])
    else:
...
def any_download(url, **kwargs):
    ...
    ffmpeg_location = kwargs['ffmpeg_location']
