        ...
        if sort == 'video':
            ...
            if pn!= len(initial_state['videoData']['pages']):#interaction video 互动视频
                ...
                for pi in range(p,pn):
                    video = {}
                    video['url'] = "https://www.bilibili.com/video/" + initial_state['bvid'] + f'?p={pi+1}&itdl_pname={quote(sanitize_title(initial_state["videoData"]["title"]))}&itdl_pindex={pi+1}&itdl_ptotal={pn}'
                    video['title'] = initial_state["videoData"]["title"] + '-p' +  str(pi)
                    videos.append(video)
                    if kwargs.get('playlist'):
                        continue
            ...
            for ep in initial_state['epList']:
                ...
                video = {}
                video['url'] = epurl + f'?itdl_pname={quote(sanitize_title(initial_state["mediaInfo"]["title"]))}&itdl_pindex={i}&itdl_ptotal={len(initial_state["epList"])}'
                video['title'] = ep['titleFormat'] + '-' + ep['longTitle']
                videos.append(video)
                if kwargs.get('playlist'):
                    continue
            ...
            initial_state_text = match1(html_content, r'__INITIAL_STATE__=(.*?);\(function\(\)')  # FIXME
            initial_state = json.loads(initial_state_text)
            season_id = str(initial_state['mediaInfo']['season_id'])

            bangumi_md_api = 'https://api.bilibili.com/pgc/web/season/section?season_id=' + season_id
            html_content = get_content(bangumi_md_api, headers=self.bilibili_headers(referer=bangumi_md_api))
            season_state = json.loads(html_content)

            epn, i = len(season_state['result']['main_section']['episodes']), 0
            for ep in season_state['result']['main_section']['episodes']:
                i += 1; # log.w('Extracting %s of %s videos ...' % (i, epn))
                
                video = {}
                video['url'] = ep['share_url'] + f'?itdl_pname={quote(sanitize_title(initial_state["mediaInfo"]["title"]))}&itdl_pindex={i}&itdl_ptotal={epn}'
                video['title'] = ep['long_title']
                videos.append(video)
                if kwargs.get('playlist'):
                    continue
            ...
            for video in channel_info['data']['list']['archives']:
                ...
                pvideo = {}
                pvideo['url'] = url + f'?itdl_pname={quote(sanitize_title(channel_info["data"]["list"]["name"]))}&itdl_pindex={i}&itdl_ptotal={epn}'
                pvideo['title'] = video['title']
                videos.append(pvideo)
                if kwargs.get('playlist'):
                    continue
            ...
            for song in menusong_info['data']['data']:
                ...
                pvideo = {}
                pvideo['url'] = url + f'?itdl_pname=bilibili-am{sid}&itdl_pindex={i}&itdl_ptotal={epn}'
                pvideo['title'] = song['title']
                videos.append(pvideo)
                if kwargs.get('playlist'):
                    continue
